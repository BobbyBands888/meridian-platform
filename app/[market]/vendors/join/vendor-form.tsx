...
export function VendorForm({ mode, brand, userId, serviceAreaExample, action, initial, submitLabel }: Props) {
  ...
  return (
    <form key={state.submittedAt ?? "initial"} action={formAction} noValidate className="space-y-7">
      ...
      <Field id="service_area" label="Service area" hint={serviceAreaExample ? `For example: ${serviceAreaExample}` : "The cities or counties you work in"} error={errors.service_area}>
