...
export async function generateMetadata({ params }: PageProps<"/[market]/vendors/[category]/[id]">): Promise<Metadata> {
  ...
  const title = `${vendor.business_name}, ${category.singular} in ${market.name}`;
  ...
  return {
    ...
    openGraph: { type: "profile", title, description, url: vendorPath(vendor), siteName: brandName(market), images },
    twitter: { card: "summary_large_image", title, description, images },
  };
}

export default async function VendorProfilePage({ params }: PageProps<"/[market]/vendors/[category]/[id]">) {
  const { market: marketSlug, id } = await params;
  const market = await requireMarket(marketSlug);
  const vendor = await getPublicVendor(market.id, id);
      ...
      <div className="mt-6 grid gap-10 lg:grid-cols-[minmax(0,1fr)_420px] lg:gap-14">
        <article>
          <div className="flex flex-col gap-6 sm:flex-row sm:items-start">
            ...
            <div>
              ...
              <ul className="mt-4 flex flex-wrap gap-x-4 gap-y-1">
                <li>
                  <Check label={vendor.verified_at ? verifiedBadgeText(market, vendor.verified_at) : "Licensed and insured, self-certified"} />
                </li>
          ...
          <p className="mt-8 rounded-xl bg-surface p-4 text-[14px] leading-relaxed text-muted">
            ...
            {vendor.verified_at
              ...
              : `${brandName(market)} hasn't reviewed this vendor's license or insurance documents.`}{" "}
        ...
        <section id="contact" aria-labelledby="contact-heading" className="scroll-mt-24 lg:sticky lg:top-24 lg:self-start">
          <div className="rounded-2xl border border-line p-6">
            ...
            <ContactForm type="vendor" targetId={vendor.id} recipientLabel={vendor.business_name} brand={brandName(market)} />
          </div>
        </section>
      </div>
    </Container>
