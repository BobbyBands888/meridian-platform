...
import { brandName, isLive, marketUrl, type Market } from "@/lib/markets";
...
const noticesFor = (market: Market): Record<string, string> => ({
  submitted: isLive(market)
    ? "Received — you'll be live within 24 hours. We sent a confirmation to your email."
    : `You're pre-registered. We'll review your profile and list you when ${brandName(market)} launches. We sent a confirmation to your email.`,
...
export default async function VendorDashboardPage({ params: routeParams, searchParams }: PageProps<"/[market]/dashboard/vendor">) {
  ...
  // One vendor profile per account, managed on its own market's site.
  const market = await getMarketById(vendor.market_id);
  if (market.id !== siteMarket.id) return <OtherMarketNotice market={market} />;

  const live = isLive(market);
  const brand = brandName(market);
  const notices = noticesFor(market);
  const notice = Object.keys(notices).find((key) => params[key] === "1");
  const inquiryDate = new Intl.DateTimeFormat("en-US", { month: "short", day: "numeric", year: "numeric", hour: "numeric", minute: "2-digit", timeZone: market.timezone });
    ...
    <Container className="py-12 sm:py-16">
      ...
      <section aria-labelledby="inquiries-heading" className="mt-8 rounded-2xl border border-line p-6">
        <h2 id="inquiries-heading" className="text-2xl font-semibold tracking-tight">
          You&apos;ve received {counts.allTime.toLocaleString("en-US")} {counts.allTime === 1 ? "inquiry" : "inquiries"} through {brand}.
        </h2>
        ...
        <section aria-labelledby="status-heading" className="rounded-2xl border border-line p-6">
          ...
          {vendor.status === "approved" && !live && (
            <p className="mt-3 text-[15px] leading-relaxed">
              <span className="font-medium">Approved for launch.</span> You&apos;ll be listed under {category.label} the day {brand}{" "}
              opens.
            </p>
          )}
          {vendor.status === "approved" && live && (
          ...
          {vendor.status === "pending" && (
            <p className="mt-3 text-[15px] leading-relaxed">
              {live ? (
                <>
                  <span className="font-medium">In review.</span> You&apos;ll get an email when your profile is live, usually within 24
                  hours.
                </>
              ) : (
                <>
                  <span className="font-medium">Pre-registered.</span> We&apos;ll review your profile before {brand} launches and email
          ...
          {vendor.founding_vendor && (
            <p className="mt-5 border-t border-line pt-4 text-[14px] leading-relaxed text-muted">
              Free for founding vendors during our {market.name} launch. When we introduce pricing, founding vendors get first
        ...
        <section aria-labelledby="profile-heading" className="rounded-2xl border border-line p-6 lg:col-span-2">
          <div className="flex items-center justify-between">
            <h2 id="profile-heading" className="text-lg font-semibold tracking-tight">
              {vendor.status === "approved" && live ? "Live profile" : "Your profile"}
            </h2>
...
function OtherMarketNotice({ market }: { market: Market }) {
  return (
    <Container className="py-12 sm:py-16">
      <Link href="/dashboard" className="text-[15px] font-medium text-forest hover:underline">
        ← Your account
      ...
      <h1 className="mt-4 text-4xl font-bold tracking-tight">Vendor dashboard</h1>
      <div className="mt-8 max-w-xl rounded-2xl border border-line p-6">
        <p className="text-[17px] leading-relaxed">
          Your vendor profile is part of <span className="font-semibold">{brandName(market)}</span>. Each account has one vendor
          profile, and you manage it on that site.
        </p>
        <a
          href={marketUrl(market, "/dashboard/vendor")}
          className="mt-5 inline-flex min-h-12 items-center rounded-lg border border-ink/15 px-6 font-semibold hover:border-forest hover:text-forest"
        >
          Go to {brandName(market)}
        </a>
      </div>
    </Container>
  );
