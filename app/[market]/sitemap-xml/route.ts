...
export async function GET(_request: Request, { params }: RouteContext<"/[market]/sitemap-xml">) {
  ...
  return sitemapResponse([
    ...
    ...marketAreas(market).filter((area) => isAreaIndexable(market, area)).map((area) => ({ url: url(`/homes/${area.slug}`), changeFrequency: "weekly" as const, priority: 0.7, lastModified: now })),
