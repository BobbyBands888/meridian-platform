...
export async function generateMetadata({ params }: PageProps<"/[market]/sign-in">): Promise<Metadata> {
  const market = await requireMarket((await params).market);
  return {
    title: "Sign in",
    description: `Sign in to ${brandName(market)} with a one-time email link. No password needed.`,
    robots: { index: false },
  };
}
...
export default async function SignInPage({ searchParams }: PageProps<"/[market]/sign-in">) {
