...
import { brandName, marketUrl } from "@/lib/markets";
...
export default async function AdminListingPage({ params, searchParams }: PageProps<"/[market]/admin/listings/[id]">) {
      ...
      <div className="mt-4 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <p className="text-[15px] text-muted">
            {brandName(market)} · Status: <span className="font-semibold text-ink">{statusLabels[listing.status]}</span>
          </p>
          <h1 className="mt-1 text-3xl font-bold tracking-tight sm:text-4xl">{listing.street}</h1>
          <p className="mt-1 text-[15px] text-muted">
            {locationLine(market, listing.zip, listing.city)} · {formatPrice(listing.price)} · {formatSpecs(listing)}
          </p>
        </div>
        {published && (
          <ButtonLink href={market.id === siteMarket.id ? listingPath(listing) : marketUrl(market, listingPath(listing))} variant="secondary">
            View public listing
          </ButtonLink>
        )}
      </div>
