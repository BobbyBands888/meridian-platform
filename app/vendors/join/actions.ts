...
export async function joinVendorDirectory(_prev: VendorFormState, formData: FormData): Promise<VendorFormState> {
  ...
  await Promise.all([sendVendorReceived(email, vendor), sendAdminNewVendor({ ...vendor, email })]);
...
export async function updateVendorProfile(_prev: VendorFormState, formData: FormData): Promise<VendorFormState> {
  ...
  if (before.status === "approved") {
    ...
    outcome = "resubmitted";
    await sendAdminNewVendor({ id: before.id, business_name: values.business_name, category: values.category as VendorCategoryValue, email }, true);
  }
