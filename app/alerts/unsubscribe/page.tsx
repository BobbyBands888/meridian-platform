...
export default async function UnsubscribePage({ searchParams }: PageProps<"/alerts/unsubscribe">) {
  ...
  const alert = isAlertToken(token)
    ? (await createAdminClient().from("listing_alerts").select("email, zip, unsubscribed_at").eq("unsubscribe_token", token).maybeSingle()).data
    : null;
        ...
        {!alert ? (
          <>
            ...
            <p className="mt-4 text-[17px] leading-relaxed text-muted">
              It may be incomplete. Open the link from your most recent Nashville Buys listing alert email, or write to us and
              we&apos;ll remove you.
            </p>
            <ButtonLink href={`mailto:${site.email}?subject=Unsubscribe%20from%20listing%20alerts`} variant="secondary" className="mt-8">
              Email {site.email}
            </ButtonLink>
          </>
          ...
          <>
            ...
            <p className="mt-4 break-words text-[17px] leading-relaxed text-muted">
              ...
              {alert.zip ? ` in ${alertAreaLabel(alert.zip)}` : ""}.
            </p>
            {error && (
