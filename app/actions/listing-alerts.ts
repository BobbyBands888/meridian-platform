...
export async function subscribeToListingAlerts(_prev: AlertSignupState, formData: FormData): Promise<AlertSignupState> {
  ...
  else if (zip && !isServiceZip(zip)) errors.zip = "That ZIP is outside the Middle Tennessee area we cover. Leave it blank to hear about every new home.";
  ...
  if (!existing) {
    const { data, error } = await admin.from("listing_alerts").insert({ email, zip: zipValue }).select("unsubscribe_token").single();
  ...
  if (confirmToken) await sendAlertConfirmation({ email, zip: zipValue, token: confirmToken });
