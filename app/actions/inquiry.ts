...
type Recipient = { email: string; subjectLabel: string; pageUrl: string; source: string };
...
async function findRecipient(type: string, targetId: string): Promise<Recipient | null> {
  ...
  if (type === "vendor") {
    const { data: vendor } = await admin
      .from("vendors")
      .select("id, category, business_name, status, profiles!inner(email)")
      .eq("id", targetId)
      .eq("status", "approved")
      .maybeSingle();
    if (!vendor) return null;
    const path = vendorPath(vendor);
    return { email: vendor.profiles.email, subjectLabel: vendor.business_name, pageUrl: siteLink(path), source: path };
  }

  if (type === "listing") {
    const { data: listing } = await admin
      .from("listings")
      .select("id, slug, street, zip, hide_exact_address, status, profiles!inner(email)")
      .eq("id", targetId)
      .eq("status", "active")
      .maybeSingle();
    ...
    return { email: listing.profiles.email, subjectLabel: label, pageUrl: siteLink(path), source: path };
  }
...
export async function sendInquiry(_prev: InquiryState, formData: FormData): Promise<InquiryState> {
  ...
  await Promise.all([
    sendEmail({
      to: recipient.email,
      replyTo: email,
      subject: `New inquiry from ${name} via Nashville Buys`,
      ...
      blocks: [
        ...
        { kind: "rows", rows },
        { kind: "p", text: "Nashville Buys doesn't take part in your conversation or any agreement you make." },
      ],
    }),
    sendEmail({
      to: adminEmail(),
      ...
      blocks: [
        { kind: "rows", rows: [["For", recipient.subjectLabel], ["Recipient", recipient.email], ...rows] },
        { kind: "quote", text: message },
      ],
    }),
  ]);
