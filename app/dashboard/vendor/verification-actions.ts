...
export async function submitVerification(_prev: VerificationFormState, formData: FormData): Promise<VerificationFormState> {
  ...
  const { data: vendor } = await supabase.from("vendors").select("id, business_name, category, status").eq("profile_id", user.id).maybeSingle();
  ...
  await sendAdminVerificationSubmitted({ id: vendor.id, business_name: vendor.business_name, email }, Boolean(existing?.submitted_at));
